...
Revision ID: f8162cd7d856
Revises: 
Create Date: 2017-12-11 01:42:59.355601
...
def upgrade():
    ...
    sa.Column('live_name', sa.String(length=128), nullable=True),
    sa.Column('live_user', sa.String(length=128), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_live_live_name'), 'live', ['live_name'], unique=True)
    op.create_index(op.f('ix_live_live_user'), 'live', ['live_user'], unique=True)
...
def downgrade():
    ...
    op.drop_table('user')
    op.drop_index(op.f('ix_live_live_user'), table_name='live')
    op.drop_index(op.f('ix_live_live_name'), table_name='live')
    op.drop_table('live')
    # ### end Alembic commands ###
